...
for(const platform of ['Android','iPhone']) {
 for(const provider of [null,'stripe','apple'])for(const occupied of [0,2,3])test(`${platform} ${provider??'Free/Featured'} slots=${occupied}`,()=>assert.equal(eventEligibility({provider,occupied,can_submit:provider!==null&&occupied<3}).allowed,provider!==null&&occupied<3));
}
