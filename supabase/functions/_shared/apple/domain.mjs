...
export const TABLES = ['buyers','installations','capabilities','challenges','catalog','grants','intents','slots','subscriptions','assignments','transactions','notifications','entitlements','deliveries','expired_purchases'];
...
export function listingFor(s, buyer, kind, id, now) {
  ...
  const grants=s.grants.filter(g=>g.buyer_id===buyer.id && g.environment===buyer.environment && g.listing_type===kind && g.listing_id===id && !g.revoked_at && time(g.created_at)<=now && time(g.expires_at)>now);
  requireThat(grants.length>0,'LISTING_UNAUTHORIZED',403);
  requireThat(grants.length===1,'LISTING_AUTHORIZATION_AMBIGUOUS',403);
  requireThat(typeof grants[0].id==='string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(grants[0].id),'LISTING_UNAUTHORIZED',403);
  requireThat(!l.stripe_conflict,'STRIPE_CONFLICT');
  return l;
}
export function authenticate(s, tokenHash, scope, now, environment='Sandbox') {
  ...
  requireThat(['Sandbox','Production'].includes(environment)&&b.environment===environment,'ENVIRONMENT_DISABLED',503);
  return {buyer:b,installation:i,capability:c};
}
// Server-created immutable purchase context. Legacy intents fail closed.
export function intentBinding(auth, i, {recovery=false}={}) {
  requireThat(i.originating_installation_id && i.app_account_token && i.bundle_id && i.subscription_group_id,'INTENT_BINDING_REQUIRED');
  requireThat(i.buyer_id===auth.buyer.id && i.environment===auth.buyer.environment && i.bundle_id===auth.buyer.bundle_id && i.app_account_token===auth.buyer.app_account_token,'INTENT_BINDING_MISMATCH');
  if(!recovery)requireThat(i.originating_installation_id===auth.installation.installation_id,'INSTALLATION_MISMATCH',403);
}
export function reserve(s, auth, input, now) {
  ...
  if(prior) {
    intentBinding(auth,prior);
    requireThat(['listing_type','listing_id','requested_plan'].every(k=>prior[k]===input[k]),'IDEMPOTENCY_CONFLICT');
    return prior;
  }
  requireThat(!s.slots.some(x=>!['released_expired','released_never_started','released_cancelled'].includes(x.state)&&x.environment===b.environment && x.listing_type===input.listing_type && x.listing_id===input.listing_id),'APPLE_CONFLICT');
  const used=new Set(s.slots.filter(x=>!['released_expired','released_never_started','released_cancelled'].includes(x.state)&&x.buyer_id===b.id && x.environment===b.environment).map(x=>x.slot_number));
  ...
  const i={id:uuid(),buyer_id:b.id,originating_installation_id:auth.installation.installation_id,app_account_token:b.app_account_token,bundle_id:BUNDLE,subscription_group_id:p.subscription_group_id,listing_type:input.listing_type,listing_id:input.listing_id,requested_plan:input.requested_plan,slot_number:slot,product_id:p.product_id,environment:b.environment,idempotency_key:input.idempotency_key,status:'reserved',created_at:iso(now),expires_at:iso(now+600000),started_at:null,completed_at:null,reconciliation_reason:null};
  s.intents.push(i);
  const occupancy={buyer_id:b.id,environment:b.environment,slot_number:slot,state:'reserved',purchase_intent_id:i.id,subscription_id:null,listing_type:i.listing_type,listing_id:i.listing_id,updated_at:iso(now)};
  const released=s.slots.find(x=>x.buyer_id===b.id&&x.environment===b.environment&&x.slot_number===slot&&['released_expired','released_never_started','released_cancelled'].includes(x.state));
  if(released)Object.assign(released,occupancy);else s.slots.push(occupancy);
  return i;
}
...
export function validateTransactionFacts(t, b, i, catalog, now) {
  requireThat(['Sandbox','Production'].includes(t.environment)&&t.environment===b.environment,'ENVIRONMENT_DISABLED',503);
  ...
  requireThat(Number.isFinite(time(t.purchase_date)) && time(t.purchase_date)<=now+30000 && time(t.expires_date)>time(t.purchase_date),'TRANSACTION_DATES');
...
export function validateTransaction(t,b,i,catalog,now) {
  const p=validateTransactionFacts(t,b,i,catalog,now);
  requireThat(time(t.expires_date)>now,'TRANSACTION_DATES');
  return p;
}
...
export function deliverApplePurchase(s,auth,i,t,hash,now) {
  intentBinding(auth,i);
  requireThat(!(s.expired_purchases??[]).some(x=>x.bundle_id===t.bundle_id&&x.environment===t.environment&&x.transaction_id===t.transaction_id),'TRANSACTION_RECONCILED_EXPIRED');
  const prior=s.transactions.find(x=>x.bundle_id===t.bundle_id && x.environment===t.environment && x.transaction_id===t.transaction_id);
  if(prior) {
    return recoverExistingDelivery(s,auth,i,t,now);
  }
...
// Only the explicit authorized recovery path may call this for another installation.
// This cannot create commercial state; it returns an already durable exact delivery.
export function recoverExistingDelivery(s,auth,i,t,now) {
  intentBinding(auth,i,{recovery:true});
  requireThat(i.status==='completed','TRANSACTION_REPLAY_CONFLICT');
  const prior=s.transactions.find(x=>x.bundle_id===t.bundle_id && x.environment===t.environment && x.transaction_id===t.transaction_id);
  requireThat(prior,'RECOVERY_REQUIRES_DELIVERY');
  listingFor(s,auth.buyer,i.listing_type,i.listing_id,now);
    requireThat(t.app_account_token===auth.buyer.app_account_token&&t.app_transaction_id===auth.buyer.app_transaction_id,'BUYER_MISMATCH');
    requireThat(t.ownership_type==='PURCHASED'&&!t.revocation_date&&!t.is_upgraded,'TRANSACTION_NOT_ELIGIBLE');
    const subscription=s.subscriptions.find(x=>x.id===prior.subscription_id);
    requireThat(subscription&&!['revoked','reconciliation'].includes(subscription.status),'DELIVERY_RECOVERY_BLOCKED');
